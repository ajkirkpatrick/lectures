...
$\sum_{\rho}^{\gamma} = 100\frac{\alpha}{\beta}$
...
- FWIW, my lecture slides and notes are all written in R Markdown too. (E.g. This slide deck is built using the [xaringan package](https://github.com/yihui/xaringan/wiki) with the metropolis theme.)

---

# Before we start

We went through a sofware installation check during the previous lecture. By now you should have:
...
Git and GitHub's role in global software develoment is not in question.
...
- Journals have increasingly strict requirements regarding reproducibility and data access. GH makes this easy (DOI integration, off-the-shelf licences, etc.)
...
The starting point for our workflow is to link a GitHub repository (i.e. "repo") to an RStudio Project. Here are the steps:
...
Practice by these steps by creating your own repo on GitHub — call it "test" — and cloning it via an RStudio Project. (GIF walkthrough on the next slide...)
...
See [here](https://github.com/richelbilderbeek/Cer2016/issues/6) if you get `Error: unable to read askpass response from 'rpostback-askpass'`.
...
- Again, they interact seemlessly with Git(Hub), as we've just seen. 
...
- P2: Clone P1's repo to your local machine.<sup>1</sup> Make some edits to the README (e.g. delete lines of text). Stage, commit and push these changes.

- P1: Now make changes to the README on your machine too. Stage, commit and then try to push them (*after* pulling from the GitHub repo first).
...
.footnote[<sup>1</sup> I also use and recommend <a href="https://atom.io/" target="_blank">Atom</a>. You can set your preferred default editor with `$ git config --global core.editor "PREFERRED_EDITOR"`.]
...
Git forks lie somewhere between cloning a repo and and branching from it.
...
- Try this now. Use of [my repos](https://github.com/grantmcdermott?tab=repositories) if you can't think of anyone else's.
...
Remember that peer-review component of the course (i.e. 10% of your final grade)? Well, now is a good time to tell you that forks, branches, and pull requests are effectively what I will be expecting of you.
